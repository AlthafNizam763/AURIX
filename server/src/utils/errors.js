 ...
 * branches on it. `AuthFailure.kind` in `api_auth_service.dart` is a switch
 * over exactly these strings, so renaming one here silently degrades a specific
 * sign-in error message into a generic one on the client.
 *
 * Anything not raised through [ApiError] becomes a 500 with the code
 * `internal`, and its message is replaced before it reaches the client — an
 * unexpected `MongoServerError` must never hand its text, which can quote a
 * query and a collection name, to a caller.
 */
 ...
 */
export const identityInUse = (provider) =>
  conflict(
    'identity_in_use',
...
export const invalidCode = () =>
  new ApiError(401, 'invalid_code', 'That code is not correct.');
 ...
 * irreversibly — an account whose last identity is gone has no sign-in path
 * and no reset path either, because a reset needs an address to send to.
 */
...
export const providerUnavailable = (provider) =>
  new ApiError(
    503,
...
export const unsupportedMedia = (message) =>
