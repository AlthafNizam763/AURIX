...
import { normaliseAlbum, tokensForPlaylist } from '../utils/search.js';
 ...
 *
 * Ported from `FirestorePlaylistService.positionBetween`, and the Dart original
 * is kept as a pure static so the existing `playlist_position_test.dart` still
 * exercises the same arithmetic on the client side.
 */
...
export function playlistOut(doc) {
  if (!doc) return null;
  return {
    ...
    // read — see the note in `db/collections.js`.
 ...
 */
export function namedFields(name) {
 ...
 * `createdAt` and gains whatever metadata improved. Duplicates within one
 * source playlist are collapsed first: two upserts on one key inside a single
 * `bulkWrite` would otherwise race each other for the same row.
 */
export async function writeTracksInOrder(collection, scope, tracks) {
  ...
  const now = new Date();
  const operations = ordered.map((entry, index) => ({
    updateOne: {
      filter: { ...scope, trackId: entry.trackId },
      update: {
        $set: {
          ...entry.track,
          ...scope,
...
export async function appendTracks(collection, scope, tracks) {
  if (tracks.length === 0) return 0;

  const last = await collection
    .find(scope, { projection: { position: 1 } })
    .sort({ position: -1 })
    .limit(1)
    .toArray();

  let cursor = last.length > 0 && typeof last[0].position === 'number' ? last[0].position : 0;
 ...
 */
export async function rebalance(collection, scope, orderedTrackIds) {
  if (orderedTrackIds.length === 0) return;
  await collection.bulkWrite(
    orderedTrackIds.map((trackId, index) => ({
      updateOne: {
        filter: { ...scope, trackId },
 ...
 * themselves are already written — so a failure here is swallowed by the
 * caller rather than failing the write that preceded it.
 */
export async function syncTrackCount(playlists, tracks, parentFilter, trackScope) {
  const count = await tracks.countDocuments(trackScope);
