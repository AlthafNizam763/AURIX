...
import { ApiError, identityInUse, lastSignInMethod, notFound } from '../utils/errors';
import { log } from '../utils/logger';
import { isApplePrivateRelay, providerLabel } from './oauth/providers';
import {
  createUser,
  localProviders,
  providersFor,
 ...
 *
 * Somebody registers with alex@example.com and a password. Six months later they
 * tap "Continue with Google" on a new phone. Naively, that is a second AURIX
 * account: an empty library, none of their playlists, and no way to discover
 * why. Naively the other way — signing them straight in because the addresses
 * match — is worse: it means anybody who can get a provider to assert an address
 ...
 *
 * Challenging always is one extra step in a flow people run once per device, and
 * it removes the whole class of problem rather than the easy half of it.
 *
...
export const LOCAL_PROVIDERS = ['password', 'phone'] as const;

export type SocialOutcome =
  | { kind: 'session'; user: UserDoc; created: boolean; linked?: boolean }
  | { kind: 'link'; user: UserDoc };

export async function identitiesFor(uid: string): Promise<IdentityDoc[]> {
  const identities = await collections.identities();
  return identities.find({ uid }).sort({ createdAt: 1 }).toArray();
}
...
export async function findIdentity(
  ...
  const identities = await collections.identities();
  return identities.findOne({ provider, subject: String(subject) });
}
 ...
 * read that precedes it: two devices completing the same link at the same moment
 * both pass any prior check and only one can pass the index. Catching 11000 here
 * is what turns that race into "already linked elsewhere" instead of a 500.
 */
export async function attachIdentity({
  ...
  try {
    ...
    if ((error as { code?: number })?.code === 11000) {
      throw identityInUse(providerLabel(provider));
    }
    throw error;
  }
}
 ...
 * because the failure mode is permanent. An account with no identity left has no
 * sign-in path and no recovery path either: a password reset needs an address to
 * send to, and there is nobody to send it to.
 */
export async function detachIdentity({
  ...
  if (provider === 'phone') {
    ...
    await users.updateOne(
      { uid },
      { $unset: { phone: '', phoneVerified: '' }, $set: { updatedAt: new Date() } },
    );
    return;
  }
  ...
  const identities = await collections.identities();
  await identities.deleteOne({ uid, provider });
}
 ...
 *  * **Apple's private relay** is excluded because it is minted per application:
 *    it will never equal another account's address, and treating it as a
 *    person's email address means storing something they would not recognise as
 *    theirs.
 *
 ...
 * match against.
 */
export function matchableEmail(profile: SocialProfile | undefined): string {
...
async function backfill(user: UserDoc, profile: SocialProfile): Promise<UserDoc> {
  ...
  // The provider just proved control of the address this account claims. That is
  // a stronger proof than the confirmation email AURIX would have sent, so it
 ...
 * method from Settings; the account is then known up front and there is nothing
 * to challenge, because holding a valid session *is* the proof.
 */
export async function resolveSocial({
  ...
  if (intent === 'link') {
    const actor = await requireUser(String(actorUid));
  ...
  if (existing) {
    ...
    // The account was deleted and the row outlived it. Clear it and carry on as
    // though this identity were new — the alternative is a sign-in that fails
    // forever with no way for the user to fix it.
    log.warn(`Identity ${provider}:${subject} pointed at a deleted account`, 'auth');
    const identities = await collections.identities();
    await identities.deleteOne({ provider, subject });
  }
  ...
  const privateRelay =
    ...
    (profile.isPrivateRelay === true || isApplePrivateRelay(storedEmail));
  ...
  try {
    ...
    if (error instanceof ApiError && (error.code === 'email_in_use' || error.status === 409)) {
 ...
 *
 * The proof itself — a password, or a code mailed to the account's address — is
 * checked by the route. This is what happens after it succeeds.
 */
...
export async function signInMethodsFor(user: UserDoc): Promise<SignInMethod[]> {
  const identities = await collections.identities();
  const rows = await identities
    .find({ uid: user.uid }, { projection: { provider: 1 } })
    .toArray();
  return [
    ...new Set<SignInMethod>([...localProviders(user), ...rows.map((r) => r.provider)]),
  ].sort();
}
