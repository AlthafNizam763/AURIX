...
import { invalidPhone } from '../utils/errors';
 ...
 * live one-time code. Both are string comparisons, so every spelling of a
 * number that reaches them has to already be the same string — otherwise
...
export function normalisePhone(raw: string | undefined): string {
...
export function isE164(raw: unknown): raw is string {
