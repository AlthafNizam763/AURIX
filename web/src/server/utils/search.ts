/**
 * A faithful port of `song_key.dart` / `SearchTokens`.
 *
 ...
 * `searchTokens` before the row is ever sent here. The *query* side runs on the
 * server, because the client sends a search string and no longer touches the
 * database.
 *
 ...
 * assert the specific pairs where the two implementations could plausibly drift
 * — accent folding, the `feat.` boundary, and stop-word handling.
 *
 * Keep this in step with `mobile/lib/data/models/song_key.dart`. It is the one
 * piece of genuinely duplicated logic in the migration, and it is duplicated
 * because the alternative — a normalisation round-trip to the server on every
...
export function normaliseArtist(value: unknown): string {
  let out = foldAccents(String(value ?? '').toLowerCase());
  ...
  const separators: (string | RegExp)[] = [
    ',',
    '&',
    /\bfeat\.?\b/,
    /\bft\.?\b/,
...
export function normaliseAlbum(value: unknown): string {
  let out = foldAccents(String(value ?? '').toLowerCase());
  out = out.replace(PACKAGING, ' ');
  out = out.replace(EMPTY_BRACKETS, ' ');
  return collapse(out);
...
function addPrefixes(field: string, into: Set<string>): void {
...
export function tokensForPlaylist(name: unknown): string[] {
  const tokens = new Set<string>();
  addPrefixes(normaliseAlbum(name), tokens);
  return [...tokens].slice(0, MAX_TOKENS);
}

function queryWords(query: unknown): string[] {
 ...
 */
export function queryToken(query: unknown): string {
...
export function residualWords(query: unknown): string[] {
  const words = queryWords(query);
 ...
 */
export function matchesResidual(haystack: string, residuals: string[]): boolean {
